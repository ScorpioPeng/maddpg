...
class Scenario(BaseScenario):
    ...
    def agent_reward(self, agent, world):
        ...
        if agent.name == 'agent 2':
            '''离防御区越近越好'''
            rew +=1*min([8/(np.sqrt(np.sum(np.square(a.state.p_pos - agent.state.p_pos)))) for a in world.forests])
    ...
    def boat_reward(selfself, boat, world):
        rew = 0
        rew -= 1 * min([np.sqrt(np.sum(np.square(a.state.p_pos - boat.state.p_pos))) for a in world.forests])
        return rew
    ...
    def observation(self, agent, world):
        ...
        for op in opponent:
           op_chi=np.zeros(1)
           if abs(op.state.p_vel[0]) < 1e-5 and abs(op.state.p_vel[1]) < 1e-5:
               op_chi[0] = 0
           else:
               op_chi[0] = math.atan2(op.state.p_vel[1], op.state.p_vel[0])
           other_chi.append(op_chi)
        ...
        if agent.adversary and not agent.leader:
            ...
            return np.concatenate([agent.state.p_vel] + [agent.state.p_pos]  + other_pos + other_vel + in_forest + [defense_pos] + our_chi + other_chi)
        if agent.leader:
